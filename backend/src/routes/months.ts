...
router.post('/', validateBody(createMonthSchema), asyncHandler(async (req: Request, res: Response) => {
  const db = getDatabase();
...
router.post('/batch', validateBody(createMonthsBatchSchema), asyncHandler(async (req: Request, res: Response) => {
  const db = getDatabase();
