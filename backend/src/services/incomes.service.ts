...
export function createIncome(
  ...
  const result = db
    ...
    .run(monthId, data.name, data.expected_date || null, data.amount || 0, data.bank_account_id || null);
...
export function receiveIncome(
  ...
  const run = db.transaction(() => {
    if (bankAccountId) {
      bankAccountsService.creditBankAccount(db, bankAccountId, existing.amount);
    }
    db.prepare(
...
export function unreceiveIncome(db: Database.Database, id: number): IncomeRow {
  ...
  const run = db.transaction(() => {
    if (existing.bank_account_id) {
      bankAccountsService.debitBankAccount(db, existing.bank_account_id, existing.amount);
    }
